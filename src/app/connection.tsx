...
export default function ConnectionScreen({ embedded = false }: { embedded?: boolean }) {
  ...
  useEffect(() => {
    void getProvisioningProfile().then(setProfile);
  }, []);
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.inputRow}><Lock size={22} color="#8C8175" /><Host style={styles.nativeInputHost} ignoreSafeArea="all"><ExpoTextInput value={nativeCode} onChangeText={(value) => { setCode(value); nativeCode.value = value; }} placeholder={i18n.t('setupCodePlaceholder')} placeholderTextColor="#8C8175" textStyle={styles.nativeInputText} autoCapitalize="none" autoCorrect={false} multiline={false} inputMode="url" textAlign="left" /></Host><Pressable onPress={() => void openScanner()} hitSlop={12}><Qr size={24} color="#8C8175" /></Pressable></View>
